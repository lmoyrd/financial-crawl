...
'''
argv_parser.add_argument('-t','--time', default='10:00', help='定时抓取时间，默认是10点') # 2 17:13
argv_parser.add_argument('-da','--download-all', default=False, type=bool, help='是否强制抓取所有数据，默认为否，只更新有变更的数据')
argv_parser.add_argument('-n','--now', default=False,  type=bool, help='是否立即开始，默认不是') # 2 17:13
spider_conf = {}
...
def load_spiders():
    ...
    spider_names = [name for name in spiders if '_detail' not in name] 
    print(spider_names)
    return spider_names
