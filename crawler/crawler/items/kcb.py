...
from ..util.constant import STOCK_EXCHANGE_TYPE, INTERMEDIARY_TYPE, KCB_STAGE, KCB_FileType_Enum,STAGE_STATUS
...
def stage_status_convert(self, retrive_dict = {}): 
    ...
    result = None
    if self.stage == KCB_STAGE.MUNICIPAL_PARTY_COMMITTEE.value:
        result = retrive_dict['commitiResult']
    if self.stage == KCB_STAGE.REGIST_RESULT.value:
        result = retrive_dict['registeResult']
    if self.stage == KCB_STAGE.SUSPEND.value:
...
def stage_name(self):
    return KCB_STAGE(self.stage).desc if self.stage and KCB_STAGE(self.stage) else ''

def stage_status_name(self):
    result = ''
        
    if self.stage_status:
        for name, member in KCB_STAGE.__members__.items():
...
"""
@attrs.define
class KCBIntermediaryItem(BaseItem):
    # 项目id
...
"""
@attrs.define
class KCBIntermediaryPersonItem(BaseItem):
    # 项目id
    project_id: str = attrs.field(default = '', metadata={'source': 'auditId'}) 
    # 中介机构id
    intermediary_id : str = attrs.field(default = '', metadata={'source': 'i_intermediaryId'}) 
...
"""
@attrs.define
class KCBCompanyMilestoneItem(BaseItem):
...
"""
@attrs.define
class KCBCompanyProcessItem(KCBCompanyMilestoneItem):
    @property
    def stage_name(self):
        return stage_name(self)
...
class KCBFileItem(BaseItem):
    ...
    def file_type_of_process_desc(self):
        return KCB_FileType_Enum(self.file_type_of_process).desc if self.file_type_of_process else ''

    @property
    def file_type_of_audit_desc(self):
        return KCB_FileType_Enum(self.file_type_of_audit).desc if self.file_type_of_audit else ''
...
"""
@attrs.define
class KCBCompanyOtherItem(KCBCompanyProcessItem):
    @property
    def stage_name(self):
        return stage_name(self)
...
"""
@attrs.define
class KCBCompanySourceItem(BaseItem):
