...
from ..util.constant import STOCK_EXCHANGE_TYPE, INTERMEDIARY_TYPE, CYB_STAGE, CYB_FileTypeOfProcess_TO_CYB_STAGE, CYBFileTypeOfAuditEnum, CYBFileTypeOfProcessEnum
...
class CYBIPOItem(BaseItem):
    ...
    @property
    def stage_name(self):
        return CYB_STAGE(self.stage).desc if self.stage and CYB_STAGE(self.stage) else ''
...
"""
@attrs.define
class CYBIntermediaryItem(BaseItem):
    # 项目id
...
"""
@attrs.define
class CYBIntermediaryPersonItem(BaseItem):
    # 项目id
    project_id: str = attrs.field(default = '', metadata={'source': 'prjid'}) 
    # 中介机构id
...
class CYBFileItem(BaseItem):
    ...
    def file_type_of_process_desc(self):
        return CYBFileTypeOfProcessEnum(self.file_type_of_process).desc if self.file_type_of_process else ''

    @property
    def file_type_of_audit_desc(self):
        return CYBFileTypeOfAuditEnum(self.file_type_of_audit).desc if self.file_type_of_audit else ''
    ...
    def stage(self):
        # 文件类型到文件所属阶段的映射
        return CYB_FileTypeOfProcess_TO_CYB_STAGE[self.file_type_of_process] if self.file_type_of_process in CYB_FileTypeOfProcess_TO_CYB_STAGE.keys() else ''
    

    @property
    def stage_name(self):
        if self.stage and CYB_STAGE(self.stage):
            return CYB_STAGE(self.stage).desc
...
"""
@attrs.define
class CYBCompanyMilestoneItem(BaseItem):
    ...
    def stage(self):
        ...
        if self.stage_status_name == '终止':
            return CYB_STAGE.END.value
        else:
            for name, member in CYB_STAGE.__members__.items():
...
"""
@attrs.define
class CYBCompanyOtherItem(BaseItem):
    @property
    def id(self):
...
"""
@attrs.define
class CYBCompanySourceItem(BaseItem):
